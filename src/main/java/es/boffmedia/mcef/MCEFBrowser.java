...
import es.boffmedia.mcef.listeners.MCEFCursorChangeListener;
...
public class MCEFBrowser extends CefBrowserOsr {
    ...
    public MCEFBrowser(MCEFClient client, String url, boolean transparent) {
        ...
        Minecraft.getInstance().enqueue(renderer::initialize);
    }
    ...
    public void onPaint(CefBrowser browser, boolean popup, Rectangle[] dirtyRects, ByteBuffer buffer, int width, int height) {
        if (!popup && (width != lastWidth || height != lastHeight)) {
            ...
            for (Rectangle dirtyRect : dirtyRects) {
                ...
                if (!popup) {
                    // Upload to the GPU
                    GlStateManager.pixelStore(GL_UNPACK_SKIP_PIXELS, dirtyRect.x);
                    GlStateManager.pixelStore(GL_UNPACK_SKIP_ROWS, dirtyRect.y);
    ...
    public void startDragging(CefDragData dragData, int mask, int x, int y) { // Overload since the JCEF method requires a browser, which then goes unused
        startDragging(null, dragData, mask, x, y);
    }
    ...
    protected void finalize() throws Throwable {
        Minecraft.getInstance().enqueue(renderer::cleanup);
        super.finalize();
    }
    ...
    public void setCursor(CefCursorType cursorType) {
        if (cursorType == CefCursorType.NONE) {
            GLFW.glfwSetInputMode(Minecraft.getInstance().getMainWindow().getHandle(), GLFW_CURSOR, GLFW_CURSOR_HIDDEN);
        } else {
