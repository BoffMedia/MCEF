...
import es.boffmedia.mcef.listeners.MCEFInitListener;
...
public final class MCEF {
    ...
    /**
     * This gets called by {@link es.boffmedia.mcef.mixins.CefInitMixin}.
    ...
    public static String getJavaCefCommit() throws IOException {
        ...
        if (!commits.isEmpty()) {
            return commits.keySet().stream().findFirst().map(commits::get).orElse(null);
        }
